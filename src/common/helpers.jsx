...
export const ConvertUTCToLocalHourAndMinuteTime = (UTCTime) => {
  ...
  const localTime = `${String(localHours).padStart(2, "0")}:${String(
    localMinutes
  ).padStart(2, "0")}`;
  return localTime;
};
...
export const tiengVietKhongDau = (input) => {
  ...
  (input || "Nguyễn Hoàng Văn Nhứt, LÊN NÚI ĐỂ KIẾM TIỀN")
    .split("")
    .forEach((char) => {
...
export function generateTableColumns123(rawTable, systemSettingTableColumns) {
  // Check if systemSettingTableColumns exists and is an array with elements
  if (
    Array.isArray(systemSettingTableColumns) &&
    systemSettingTableColumns.length > 0
  ) {
    ...
    const tableColumns = systemSettingTableColumns.map((setting) => ({
      ...
      dataIndex: setting.col_table_column, // Use the column name as the dataIndex
      key: setting.col_table_column, // Use the column name as the unique column key
      ...
      dataType: setting.datetime_type
        ...
        ? "number"
        : "string", // Default to 'string' if no specific type is provided
...
export const formatEmployeeNumberManager = (text) =>
  String(text)?.replace(/[()]/g, "");

export const checkRole = (requiredRoles, userRoles) => {
...
export function buildFullPath({
  ...
  if (!record || !record.SystemObjectRcd) {
    return "Invalid Record";
  }

  // Lookup object hiện tại
  const objectMap = new Map(
    dataSystemObjectVisible.map((o) => [o.SystemObjectRcd, o])
  );
  const object = objectMap.get(record.SystemObjectRcd);

  // Nếu không tìm thấy object thì return fallback
  if (!object) {
    return `Unknown Object/${
      record?.Title || record?.SystemObjectOperationRcd || "Unnamed Function"
    }`;
  }

  // Lookup category
  const category = dataSystemObjectCategoryVisible.find(
    (c) => c.SystemObjectCategoryRcd === object.SystemObjectCategoryRcd
  );
  const categoryName = category?.NameL || category?.NameE || "Unknown Category";

  const objectPath = [];
  let currentObject = object;

  // Duyệt lên theo cha để xây path
  while (currentObject) {
    objectPath.unshift(
      currentObject.SystemObjectNameE ||
        currentObject.SystemObjectName ||
        "Unnamed Object"
    );
    const parentKey = currentObject.ParentSystemObjectRcd;
    if (parentKey && objectMap.has(parentKey)) {
      currentObject = objectMap.get(parentKey);
    } else {
      break;
    }
  }
  ...
  return [categoryName, ...objectPath, functionName].join("/");
}

export function renderNewStatusTag(grantRightsFlag, isSelected) {
  let newStatus = grantRightsFlag;
  if (isSelected) {
    newStatus = !grantRightsFlag;
  }
  let color = newStatus === true ? "green" : "volcano";
  let nameStatus = newStatus === true ? "Grant" : "Not grant";
  return (
    <Tag color={color} key={String(newStatus)}>
